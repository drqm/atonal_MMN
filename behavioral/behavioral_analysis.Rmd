...
In this document, we show the results of an analysis including the 39 participants of the behavioral study who fulfilled the strict inclusion criteria. 
...
The results show that participants were less confident in the atonal condition. There was not a significant condition*deviance interaction, but participants still tended to give particularly lower ratings to the standard melodies in the atonal condition.

Given this result, we explore whether a similar pattern can be seen for deviance detection. We therefore run a logistic regression on accuracy, with condition (tonal/atonal), deviance (standard/deviant) and their interaction as predictors.
```{r}
...
Notably, the same (non-significant) trend can be seen here. Participants tended to be less accurate in atonal melodies, and more so when the melody was a standard. Nevertheless, note that accuracy was already lower for deviant melodies.
...
ggsave("joint_plots.png", plot=joint_plots,width = 180, height = 180,
       units = 'mm', dpi = 600)
joint_plots
